...
def save_comparison_plot(y_true, y_pred, title, save_plots_path="./plots",exp_i=0):
    ...
    out_path = os.path.join(out_dir, f"{exp_i}_exp_{title}_comparison_plot.png")
...
def evaluate_best_model(test_loader, best_checkpoint_path, model, device="cpu",num_samples=6,exp_i=0):
    ...
    for i in range(min(num_samples, len(all_trues))):  # 选择前num_samples个样本进行保存
        save_comparison_plot(all_trues[i], all_preds[i], f"{exp_i}_exp_Sample_{i+1}", save_plots_path="./plots")
...
def save_loss_to_csv(exp_id, epoch, train_loss_avg, val_loss_avg, save_dir="./logs"):
    ...
    # 定义 CSV 文件路径
    csv_file_path = os.path.join(save_dir, f"experiment_{exp_id + 1}_loss_results.csv")
...
def train_lstm_transformer_model(train_loader, val_loader, model, num_epochs=10, lr=1e-3, device="cpu", checkpoint_dir="./checkpoints", patience=10,exp_i=0):
    ...
    for epoch in range(num_epochs):
        ...
        # 保存当前 epoch 的检查点
        checkpoint_path = os.path.join(checkpoint_dir, f"lstm_transformer_{exp_i}_exp.pth")
        torch.save({
            'epoch': epoch + 1,
        ...
        if val_loss_avg < best_val_loss:
            ...
            best_checkpoint_path = os.path.join(checkpoint_dir, f"best_checkpoint_{exp_i}.pth")
...
def run_experiment(train_df, test_df, target_col="cnt", input_size=96, output_size=96, batch_size=32, num_epochs=10, device="cpu", exp_i=0):
    # 数值特征和类别特征
